...
class PocReg(RegistrationMethod):
    def __init__(self):
        RegistrationMethod.__init__(self)
        self.method_name = 'POC'
        self.FITTING_SHAPE = (8, 8)
        self.EPS = 0.001
    ...
    def find_shift(self, ref_image, cmp_image):
        ...
        mc = np.array([self.FITTING_SHAPE[0] / 2.0, self.FITTING_SHAPE[1] / 2.0])
        fitting_area = poc[int(peak[0] - mc[0]): int(peak[0] + mc[0] + 1),
                       int(peak[1] - mc[1]): int(peak[1] + mc[1] + 1)]
        if fitting_area.shape != (self.FITTING_SHAPE[0] + 1, self.FITTING_SHAPE[1] + 1):
            return 0.0, 0.0, 0.0
        ...
        p0 = np.array([0.0, -(peak[0] - m[0]) - self.EPS, -(peak[1] - m[1]) - self.EPS])
